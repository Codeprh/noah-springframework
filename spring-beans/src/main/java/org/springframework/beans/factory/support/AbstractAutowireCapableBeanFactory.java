...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		try {
			//Noah:aop入口接入：替换常规创建bean，直接走用户自定义创建的bean过程
	...
	protected Object doCreateBean(final String beanName, final RootBeanDefinition mbd, final @Nullable Object[] args)
		...
		if (instanceWrapper == null) {
			...
			//noah：这是就是为什么构造器依赖注入，不支持循环依赖的问题，因为这里还没有使用到缓存
		...
		if (earlySingletonExposure) {
			...
			if (earlySingletonReference != null) {
				//noah：被增强后的bean保存在缓存中，返回的是原来的bean。beanPostProcessor在每个责任链处理完之后应该要返回原对象，增强对象保存在缓存中
	...
	protected Object resolveBeforeInstantiation(String beanName, RootBeanDefinition mbd) {
		...
		if (!Boolean.FALSE.equals(mbd.beforeInstantiationResolved)) {
			...
			if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
				...
				if (targetType != null) {
					//InstantiationAwareBeanPostProcessor非常规创建bean
					bean = applyBeanPostProcessorsBeforeInstantiation(targetType, beanName);
					if (bean != null) {
						//BeanPostProcessor：精华，aop的顺序执行代码原理
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		if (resolvedAutowireMode == AUTOWIRE_BY_NAME || resolvedAutowireMode == AUTOWIRE_BY_TYPE) {
			...
			// 	<bean id="boyFriend" class="com.noah.dao.impl.BoyFriend"  autowire="byName"></bean>
			if (resolvedAutowireMode == AUTOWIRE_BY_NAME) {
				autowireByName(beanName, mbd, bw, newPvs);
			}
			// Add property values based on autowire by type if applicable.
			//根据Bean的类型进行autowiring自动装配处理
			//	<bean id="boyFriend" class="com.noah.dao.impl.BoyFriend"  autowire="byType"></bean>
	...
	protected void invokeInitMethods(String beanName, final Object bean, @Nullable RootBeanDefinition mbd)
		...
		if (mbd != null && bean.getClass() != NullBean.class) {
			...
			//才能执行用户配置的方法
			//样例<bean id="person" class="com.noah.Person"  init-method="init" destroy-method="destory"/>
