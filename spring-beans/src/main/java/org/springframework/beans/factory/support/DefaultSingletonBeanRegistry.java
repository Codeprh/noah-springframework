...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
	...
	/**
	 * 1、noah：只有singletonObjects使用了concurrentHashMap,后面的map都是使用HashMap，
     * 看使用sysnchronized同步锁。
     *
     * 2、noah：三级缓存精华，只bean实例只存在这三级缓存中的其中一级中
     *
     *
			...
			synchronized (this.singletonObjects) {
				...
				if (singletonObject == null && allowEarlyReference) {
					...
					if (singletonFactory != null) {
						...
						//noah：aop调用的地方，增强bean，在创建bean的时候，并没有真的执行bean增强，而是创建完bean之后放入了三级缓存
	...
	public void registerDependentBean(String beanName, String dependentBeanName) {
		...
		synchronized (this.dependentBeanMap) {
			...
			//比如当前需要实例化的是Bean的名字是userInfo,userInfo(dependentBeanName)中有个Human类型的属性human，
			// 那么就有human(beanName)被userInfo引用的关系 human=[userInfo]
