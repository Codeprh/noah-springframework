...
public class ProxyConfig implements Serializable {
    ...
    /**
     * 标记是否对代理进行优化。
     * true :如果Bean有接口就直接使用JDK动态代理,没有接7就使用CGLIB
     */
    private boolean optimize = false;
    /**
     * 标记是否需要阻止通过该配置创建的代理对象转换为Advised类型,默认值为false ,表示代理对象可以被转换为Advise类型
     * Advised接口其实就代表了被代理的对象(此接口是Spring AOP提供,它提供了方法可以对代理进行操作,
     * 比如移除一个切面之类的) ,它持有了代理对象的一些属性,通过它可以对生成的代理对象的一些属性进行人为干预
     * 默认情况,我们可以这么用Advised target = (Advised) context.getBean( "opaqueTest ");从而就可以对该代理持有的一些属性
     */
    boolean opaque = false;
    /**
     * 标记代理对象是否应该被aop框架通过AopContext以ThreadLocal的形式暴露出去。
     * 当一个代理对象需要调用它[proxy本身]的另外一个代理方法时 ,这个属性将非常有用。默认是是false ,以避免
     */
